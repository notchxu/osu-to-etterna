...
local t =
	...
	JudgmentMessageCommand = function(self, msg)
		...
		wifey = math.max(0, Floor(msg.WifePercent * 100) / 100)
		jdgct = msg.Val
...
local barDuration = 10 -- Time duration in seconds before the ticks fade out. Doesn't need to be higher than 1. Maybe if you have 300 bars I guess.
	...
	Def.Quad {
		...
		InitCommand = function(self)
			if enabledErrorBar == 2 then
				self:xy(MovableValues.ErrorBarX, MovableValues.ErrorBarY):zoomto(barWidth, MovableValues.ErrorBarHeight):diffusealpha(
					1
				):diffuse(color("#FFFFFF")) --diffuse(getMainColor("enabled"))
			else
				self:visible(false)
			end
		end,
